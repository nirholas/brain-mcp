// Tool-surface invariants for @three-ws/brain-mcp.
//
...
//
// Run: node --test packages/brain-mcp/test/registration.test.mjs
...
const EXPECTED_NAMES = ['list_providers', 'chat'];
...
test('every tool is a read-only, live-data query', () => {
	for (const tool of TOOLS) {
		...
		// A completion is non-deterministic and the provider set moves between
		// calls — neither tool is idempotent.
